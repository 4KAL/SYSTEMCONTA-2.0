use rusqlite::{Connection, params, types::Value, TransactionBehavior};
use std::path::Path;
use std::time::Instant;

const PYTHON_DB: &str = "contabilidad.db";
const RUST_DB: &str = "contabilidad_rust.db";
...
fn main() {
    let py = std::env::var("CONTA_SRC").unwrap_or_else(|_| PYTHON_DB.to_string());
    let rust_path = std::env::var("CONTA_DST").unwrap_or_else(|_| RUST_DB.to_string());
    if !Path::new(&py).exists() {
        eprintln!("Error: No se encuentra el origen '{}'", py);
        std::process::exit(1);
    }
    if Path::new(&rust_path).exists() {
        println!("Eliminando base de datos destino anterior ({} bytes)...",
            std::fs::metadata(PYTHON_DB).map(|m| m.len()).unwrap_or(0));
        std::fs::remove_file(&rust_path).ok();
    }

    println!("=== Migración: Python → Rust ===");
    println!("Origen:  {}", py);
    println!("Destino: {}", rust_path);
    println!();

    let start = Instant::now();

    let py = Connection::open(&py).expect("Error al abrir Python DB");
    let rust = Connection::open(&rust_path).expect("Error al crear Rust DB");
    ...
    // Python 'ahorro' is movement log → migrate to ahorro_movimientos
    if table_has_rows(&py, "ahorro") {
        ...
        total += 1;
        println!("  ahorros                         cuenta general creada");
        total += migrate_ahorro_movimientos(&py, &rust);
    }

    rust.execute_batch("PRAGMA foreign_keys=ON;").ok();

    // Limpiar NULLs de columnas TEXT que los modelos leen como String (evita listas vacías).
    // Las columnas numéricas NULL se dejan como NULL: los modelos las leen como Option.
    ...
    // Reconstruir saldos a partir de pagos recibidos, movimientos y gastos
    // (la base antigua no almacenaba saldo_pendiente / saldo_actual).
    ...
    if upd_ventas.unwrap_or(0) > 0 { println!("  saldos recalculados en ventas"); }
    if upd_clientes.unwrap_or(0) > 0 { println!("  saldos recalculados en clientes"); }

    // Proveedores: deuda = gastos totales menos pagos realizados.
    ...
    // Cuentas de crédito: saldo = último saldo_acumulado de movimientos.
    ...
    if upd_prov.unwrap_or(0) > 0 { println!("  saldos recalculados en proveedores"); }
    if upd_cred.unwrap_or(0) > 0 { println!("  saldos recalculados en cuentas_credito"); }

    // Volcar deudas pendientes a deudas_empresa (origen no tenía la tabla).
    ...
    if n_deudas.unwrap_or(0) > 0 { println!("  deudas_empresa generadas: {}", n_deudas.unwrap()); }

    limpiar_nulls_texto(&rust);

    let elapsed = start.elapsed();
    println!("\n=== Migración completada ===");
    println!("  Filas migradas: {}", total);
    println!("  Tiempo: {:.2}s", elapsed.as_secs_f64());
    verificar_conteos(&py, &rust);
    println!("\nElimina '{}' y renombra '{}' si todo está bien.", PYTHON_DB, RUST_DB);
}

fn verificar_conteos(py: &Connection, rust: &Connection) {
    // (tabla_origen, columna_clave_origen, tabla_destino, columna_clave_destino)
    // Si la columna clave está definida y la tabla destino la exige UNIQUE,
    // se compara COUNT DISTINCT (origen) contra COUNT (destino).
    ...
    for (o, ok, d, dk) in pares {
        ...
        println!("  {:26} {:>5} → {:>5}  {}", format!("{}.{}", o, d), oc, dc, estado);
    }
    if hay_error {
